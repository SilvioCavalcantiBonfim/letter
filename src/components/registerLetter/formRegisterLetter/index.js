import react from "react";
import styled from "styled-components";
import { IconAddPicture, IconClose, IconPenNib, IconRefresh, IconWarning } from "../../../icons";
import GenericalCard from "../../genericalCard";
import { StyledActions, StyledInput, StyledInputDesable, StyledModalLetter, StyledTextcontent } from "./style"

const FormRegisterLetter = (props) => {
    const [values, setValues] = react.useState({title: '', image: "data:image/png;base64,", body: '', pseudonyms: null});
    const [seedPseudonyms, setSeedPseudonyms] = react.useState(null);
    const [FocusInput, setFocusInput] = react.useState(0);
   
    const [isValid, setIsValid] = react.useState({title: 1, body: 1});

    react.useEffect(() => {
        const TimeTitle = isValid.title === 0 && setInterval(() => {
            setIsValid(v => {return {...v,title: 1}});
        }, 3000);
        const TimeBody = isValid.body === 0 && setInterval(() => {
            setIsValid(v => {return {...v,body: 1}});
        }, 3000);
        return () => [isValid.title === 0 && clearInterval(TimeTitle), isValid.body === 0 && clearInterval(TimeBody)];
    },[isValid]);

    const SelectImage = react.useRef(null);

    react.useEffect(() => {
        seedPseudonyms !== null && getPseudonyms(seedPseudonyms);
    }, [seedPseudonyms]);


    const getPseudonyms = async (seed) => {
        await fetch('https://middleware-hazel.vercel.app/REQUEST', { headers: { endpoint: `http://randomuser.me/api?nat=uk,us,br&inc=name&seed=${seed}&noinfo` } }).then(async r => {
            const result = await r.json();
            setValues(v => {return {...v,pseudonyms: `${result.results[0].name.title} ${result.results[0].name.first} ${result.results[0].name.last}`}})
        })
    }

    react.useEffect(() => {
        if (localStorage.getItem('token') === null)
            localStorage.setItem('token', Math.random().toString(36).substring(2));
        setSeedPseudonyms(localStorage.getItem('token'));
    }, []);

    const updateSeed = (seed) => {
        localStorage.setItem('token', seed);
        setSeedPseudonyms(seed);
    }

    return (<form onSubmit={(e) => {
        e.preventDefault();
        setIsValid({title: Number(values.title !== ''), body: Number(values.body !== '')});
    }}>
        <StyledModalLetter>
            <div className="card">
                <div className="header">
                    <div className="content">
                        <div className="monogram"><IconPenNib /></div>
                        <div className="text">
                            <div className="text-header"/>
                        </div>
                        <div className="icon-button">
                            <button type="button" className="iconButton"><IconClose /></button>
                        </div>
                    </div>
                </div>
                <div className="media">
                    <div className="hoverMedia" onClick={() => SelectImage.current.click()}>
                        <IconAddPicture/>
                    </div>
                    <img className="defualtMedia" src={values.image}/>
                    <input type='file' className="ImageSelectInput" ref={SelectImage} accept="image/*"
                        onChange={async (e) => {
                            console.log(e.target.files);
                            const reader = new FileReader();
                            reader.onloadend = () => setValues(v => {return {...v, image: reader.result}});
                            reader.readAsDataURL(e.target.files[0]);
                        }}
                    />
                </div>
                <div className="Textcontent">
                    <div className="supportingText">
                        <StyledInput
                            isvalid={isValid.title}
                            content='Titulo'
                            spaceBottom='10px'
                            clear={(values.title === '' && FocusInput === 0) ? 1 : 0}>
                            <input name="title" value={values.title} onFocus={() => setFocusInput(1)} onBlur={() => setFocusInput(0)} onChange={(e) => setValues(v => {return {...v,title: e.target.value}})} />
                            <div className="alert"><IconWarning/></div>
                        </StyledInput>
                        <StyledInputDesable content='autor(a)' spaceBottom='32px'>
                            <input name="title" defaultValue={values.pseudonyms} disabled />
                            <button className="buttonInput" type="button" onClick={() => updateSeed(Math.random().toString(36).substring(2))}><IconRefresh /></button>
                        </StyledInputDesable>
                        <StyledTextcontent isvalid={isValid.body}>
                            <textarea onChange={(e) => setValues(v => {return {...v,body: e.target.value}})} />
                        </StyledTextcontent>
                    </div>
                    <div className="actions">
                        <StyledActions><input type="submit" value="Salvar" /></StyledActions>
                    </div>
                </div>
            </div>
        </StyledModalLetter>
    </form>);
}

export default FormRegisterLetter;