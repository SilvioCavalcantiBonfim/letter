import react from "react";
import { IconAddPicture, IconClose, IconPenNib, IconRefresh, IconWarning } from "../../../icons";
import { SupaBase } from "../../../supabase";
import { NotificationContext } from "../../notification/NotificationSystem";
import { StyledActions, StyledInput, StyledInputDesable, StyledModalLetter, StyledTextcontent } from "./style"

const FormRegisterLetter = (props) => {
    const contextNot = react.useContext(NotificationContext);
    // Controle do formulario
    const [values, setValues] = react.useState({
        title: '', 
        image: "data:image/png;base64,", 
        text: '', 
        pseudonyms: null});
        
    //validação do formulário
    const [isValid, setIsValid] = react.useState({title: 1, text: 1});
    
    react.useEffect(() => {
        const TimeTitle = isValid.title === 0 && setInterval(() => {
            setIsValid(v => {return {...v,title: 1}});
        }, 800);
        return () => isValid.title === 0 && clearInterval(TimeTitle);
    },[isValid.title]);

    react.useEffect(() => {
        const TimeBody = isValid.text === 0 && setInterval(() => {
            setIsValid(v => {return {...v,text: 1}});
        }, 800);
        return () => isValid.text === 0 && clearInterval(TimeBody);
    },[isValid.text]);

    // Controle do pseudônimo
    const [seedPseudonyms, setSeedPseudonyms] = react.useState(null);
    
    const updateSeed = (seed) => {
        localStorage.setItem('token', seed);
        setSeedPseudonyms(seed);
    }

    react.useEffect(() => {
        if (localStorage.getItem('token') === null)
            localStorage.setItem('token', Math.random().toString(36).substring(2));
        setSeedPseudonyms(localStorage.getItem('token'));
    }, []);

    react.useEffect(() => {
        seedPseudonyms !== null && getPseudonyms(seedPseudonyms);
    }, [seedPseudonyms]);

    const getPseudonyms = async (seed) => {
        await fetch('https://middleware-hazel.vercel.app/REQUEST', { headers: { endpoint: `http://randomuser.me/api?nat=uk,us,br&inc=name&seed=${seed}&noinfo` } }).then(async r => {
            const result = await r.json();
            setValues(v => {return {...v,pseudonyms: `${result.results[0].name.title} ${result.results[0].name.first} ${result.results[0].name.last}`}})
        })
    }
    //
    const [FocusInput, setFocusInput] = react.useState(0);
    const [exit, setExit] = react.useState(0);
   

    react.useEffect(() => {
        const TimeExit = exit === 1 && setInterval(() => {
            props.HandleCloseForm();
        }, 250);
        return () => isValid.text === 1 && clearInterval(TimeExit);
    },[exit]);

    const SelectImage = react.useRef(null);


    // Buttom update Pseudonyms
    const [stateButton, setStateButton] = react.useState(0);

    react.useEffect(() => {
        const TimeState = stateButton === 1 && setInterval(() => {
            setStateButton(0);
        }, 300);
        return () => stateButton === 1 && clearInterval(TimeState);
    },[stateButton]);

    return (<form onSubmit={(e) => {
        e.preventDefault();
        setIsValid({title: Number(values.title !== ''), text: Number(values.text !== '')});
        if(Number(values.title !== '')+Number(values.text !== '') === 2){
            SupaBase.from('letters').insert({...values}).then((r) => {
                contextNot.add({ id: Date.now(), text: `${values.title} adicionado.`, type: 1 });
                console.log(r)
            }).catch(error => contextNot.add({ id: Date.now(), text: `${error.toString()}`, type: 0 }));
            setExit(1);
        }
    }}>
        <StyledModalLetter state={exit}>
            <div className="card">
                <div className="header">
                    <div className="content">
                        <div className="monogram"><IconPenNib /></div>
                        <div className="text">
                            <div className="text-header"/>
                        </div>
                        <div className="icon-button">
                            <button type="button" className="iconButton" onClick={() => setExit(1)}><IconClose /></button>
                        </div>
                    </div>
                </div>
                <div className="media">
                    <div className="hoverMedia" onClick={() => SelectImage.current.click()}>
                        <IconAddPicture/>
                    </div>
                    <img className="defualtMedia" src={values.image}/>
                    <input type='file' className="ImageSelectInput" ref={SelectImage} accept="image/*"
                        onChange={async (e) => {
                            console.log(e.target.files);
                            const reader = new FileReader();
                            reader.onloadend = () => setValues(v => {return {...v, image: reader.result}});
                            reader.readAsDataURL(e.target.files[0]);
                        }}
                    />
                </div>
                <div className="Textcontent">
                    <div className="supportingText">
                        <StyledInput
                            isvalid={isValid.title}
                            content='Titulo'
                            spaceBottom='10px'
                            clear={(values.title === '' && FocusInput === 0) ? 1 : 0}>
                            <input name="title" value={values.title} onFocus={() => setFocusInput(1)} onBlur={() => setFocusInput(0)} onChange={(e) => setValues(v => {return {...v,title: e.target.value}})} />
                            <div className="alert"><IconWarning/></div>
                        </StyledInput>
                        <StyledInputDesable content='autor(a)' spaceBottom='32px' stateButton={stateButton}>
                            <input name="title" defaultValue={values.pseudonyms} disabled />
                            <button className="buttonInput" type="button" onClick={() => {updateSeed(Math.random().toString(36).substring(2));setStateButton(1);}}><IconRefresh /></button>
                        </StyledInputDesable>
                        <StyledTextcontent isvalid={isValid.text}>
                            <textarea onChange={(e) => setValues(v => {return {...v,text: e.target.value}})} />
                        </StyledTextcontent>
                    </div>
                    <div className="actions">
                        <StyledActions><input type="submit" value="Salvar" /></StyledActions>
                    </div>
                </div>
            </div>
        </StyledModalLetter>
    </form>);
}

export default FormRegisterLetter;